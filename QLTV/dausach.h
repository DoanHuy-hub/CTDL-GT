...
void DrawDanhMucSach() {
	...
	outtextxy(XDMS[0] + 20, 920, "Them sach: Chi them toi da 99 sach");
}
...
void DrawThemDauSach() {
	...
	bar(XDS[6]+15, 150, w-15, 215);

	setcolor(BORDER_COLOR);
	setlinestyle(SOLID_FILL, 0, 3);
	rectangle(XDS[6]+20, 150, w-15, 750);
	line(XDS[6]+20, 215, w-15, 215);
	line(XDS[6]+20, 650, w-15, 650);

	settextstyle(BOLD_FONT, HORIZ_DIR, 3);
	setcolor(TEXT_TITLE_COLOR);
	outtextxy((w-15+XDS[6]+20)/2 - textwidth(ThemDauSach)/2, 180 - textheight(ThemDauSach)/2, ThemDauSach);
...
void DrawHieuChinhDauSach() {
	...
	settextstyle(BOLD_FONT, HORIZ_DIR, 3);
	setcolor(TEXT_TITLE_COLOR);
	outtextxy((w-15+XDS[6]+20)/2 - textwidth(HieuChinhDauSach)/2, 180 - textheight(HieuChinhDauSach)/2, HieuChinhDauSach);
...
void AutoCreateMaSach() {
	if(curNhapSach == 1) startIndexSach = DSDS.nodes[curDauSach]->masachtudong;
	else startIndexSach++;
...
bool CheckXoaDauSach() {
	int i = ExistsDauSach(DSDS, edHieuChinhISBN.content);
	DauSach* ds = DSDS.nodes[i];
			...
			if(Window == THEM_DAU_SACH) {
				...
				if(btnHieuChinhDauSach.isMouseHover(mx, my)) {
					...
					if(CheckHieuChinhDauSach()) {
						bool check = confirm("Ban co chac muon", "hieu chinh dau sach nay?");
						if(check) {
				...
				if(btnXoaDauSach.isMouseHover(mx,my)) {
					if(!CheckXoaDauSach() ) {
						printf("confirm");
						bool check = confirm("Ban co chac muon", "xoa dau sach nay?");
						if(check) {
							DeleteDauSach(DSDS, edHieuChinhISBN.trim());
							strcpy(mess, "Xoa dau sach thanh cong!");
							ClearContentHieuChinhDauSach();

						}
					} else strcpy(mess, "Khong the xoa do dau sach da co sach!");
				...
				if(subWindow == NHAP_SACH) {
					...
					if(btnThemSach.isMouseHover(mx, my)) {
						...
						if(CheckSach()) {
							...
							DSDS.nodes[curDauSach]->masachtudong ++;
					...
					if(btnHieuChinhSach.isMouseHover(mx, my)) {
						bool check = confirm("Ban co chac muon", "hieu chinh sach nay?");
						if(check) {
						...
						bool check = confirm("Ban co chac muon", "xoa sach nay?");
						if(check) {
							NodeSach *nodeSelect = GetNodeSach(DSDS.nodes[curDauSach]->First, 10*(curPageSach-1) + curSach);
							if(nodeSelect->sach.trangthai==0) {
								bool deleted =	DeleteNodeSach(DSDS.nodes[curDauSach]->First, edHieuChinhMaSach.content);
								if(deleted) {
									DSDS.nodes[curDauSach]->soluong--;
									strcpy(mess, "Xoa sach thanh cong!");
								} else strcpy(mess, "Khong the xoa");
							} else if(nodeSelect->sach.trangthai==1) {
								strcpy(mess, "Khong the xoa sach da cho muon!");
							} else{
								strcpy(mess, "Khong the xoa sach da thanh ly!");
							}
						}	
						ClearContentHieuChinhSach();					
						ClearRightLayout();
...
void DrawList() {
	...
	// tips
	setcolor(TIPS);
	rectangle(XDS[0], 850, XDS[6], 980);
	outtextxy(30, 860, "Click chuot trai: Hieu chinh dau sach");
	outtextxy(30, 900, "Click chuot phai: Xem thong tin, danh muc sach");
	outtextxy(30, 940, "Them dau sach: So trang va Nam xuat ban chi duoc nhap so");
}
