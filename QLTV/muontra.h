...
void ItemMTEvent(){
	if(mx > XMT[0] && mx < XMT[4] && my > 700-8 && my < 820-8){
		
		if(curItemMT != GetItemMTPosition(my)){
			if(curItemMT != -1){
				// khoi phuc item
				setfillstyle(SOLID_FILL, BG_COLOR);
				bar(XMT[0], 700 + curItemMT*40 - 8, XMT[4], 700+(curItemMT+1)*40-8);
...
void DrawItemMT(int i){
	...
	outtextxy(XMT[0]+10, 700+i*40, DSMT.mt[i].MASACH);
	outtextxy(XMT[1]+10, 700+i*40, DSMT.mt[i].TenSach);
	outtextxy(XMT[2]+10, 700+i*40, DSMT.mt[i].NgayMuon);
	outtextxy(XMT[3]+10, 700+i*40, TTMuonTra[DSMT.mt[i].TrangThai]);
}
...
void DrawThongTinDocGia(){
	...
	if(curDGMT == NULL){
		setcolor(TIPS);
		outtextxy(140, 300, "Khong tim thay doc gia nay");
	}else{	
		char ch[30];
		settextstyle(BOLD_FONT, HORIZ_DIR, 2);
		setcolor(TEXT_COLOR);
		outtextxy(150, 300, "Doc gia :");
		sprintf(ch, "%s %s", curDGMT->ho, curDGMT->ten);
		outtextxy(450, 300, ch);
		
		outtextxy(150, 350, "Ma the :");
		sprintf(ch, "%d", curDGMT->MATHE);
		outtextxy(450, 350, ch);
		
		outtextxy(150, 400, "Phai :");
		outtextxy(450, 400, PhaiDocGia[curDGMT->phai]);
		
		outtextxy(150, 450, "Trang thai the :");
		outtextxy(450, 450, TTTDocGia[curDGMT->trangthai]);
		
		settextstyle(BOLD_FONT, HORIZ_DIR, 3);
		setcolor(TEXT_TITLE_COLOR);
		outtextxy((XMT[0]+XMT[4])/2 - textwidth(DSDM)/2, 550, DSDM);
...
//		}
		...
		if(curMenuMuonTra == btnMuonSach.id){
			...
			if(curDGMT->trangthai == 0){
				// the bi khoa
				outtextxy(1100, 350, "THE DOC GIA DANG BI KHOA, KHONG THE MUON SACH");
				return;
			}
			if(DSMT.n >= 3){
				outtextxy(1100, 350, "SO LUONG SACH MUON DA DAT GIOI HAN !");
				outtextxy(1100, 400, "VUI LONG TRA SACH TRUOC KHI MUON SACH MOI");
				return;
			}
			if(isQH){
				outtextxy(1100, 350, "DOC GIA DA MUON SACH QUA 7 NGAY ");
				outtextxy(1100, 400, "VUI LONG TRA SACH TRUOC KHI MUON SACH MOI");
				return;
			}
...
void DrawThongTinSachTra(int itemMT){
	if(itemMT != -1){
		...
		settextstyle(BOLD_FONT, HORIZ_DIR, 3);
		setcolor(TEXT_TITLE_COLOR);
		outtextxy(1300, 250, "THONG TIN SACH");
		
		settextstyle(BOLD_FONT, HORIZ_DIR, 2);
		setcolor(TEXT_COLOR);
		
		outtextxy(1200, 300, "Ma sach :");	
		outtextxy(1500, 300, DSMT.mt[curMT].MASACH);
		
		outtextxy(1200, 350, "Ten sach : ");
		outtextxy(1500, 350, DSMT.mt[curMT].TenSach);
		
		outtextxy(1200, 400, "Ngay muon :");
		outtextxy(1500, 400, DSMT.mt[curMT].NgayMuon);
...
void DrawLamMatSach(){
	...
	outtextxy(1200, 600, "Sach se chuyen qua trang thai: DA LAM MAT");
...
void DrawThongTinSach(){
	...
	if(curSachMT != NULL){
		outtextxy(1100, 300, "Ma sach :");	
		outtextxy(1400, 300, curSachMT->MASACH);
		
		outtextxy(1100, 350, "Ten sach : ");
		outtextxy(1400, 350, curDSMT->tensach);
		
		outtextxy(1100, 400, "Trang thai :");
		outtextxy(1400, 400, TrangThaiSach[curSachMT->trangthai]);
		
		outtextxy(1100, 450, "Vi tri :");
		outtextxy(1400, 450, curSachMT->vitri);
		
		if(curSachMT->trangthai == 0){
			outtextxy(1200, 500, "=> SACH CO THE MUON !");
			canMT = true;	// Co the muon sach
		}else if(curSachMT->trangthai == 1){
			setcolor(TIPS);
			outtextxy(1200, 500, "=> SACH DA CO NGUOI MUON !");
			return;
		}else{
			setcolor(TIPS);
			outtextxy(1200, 500, "=> SACH DA BI MAT, KHONG THE MUON !");
			return;
		}
...
//		strcpy(edNhapNgayMuonSach.content, GetSystemDate());
		...
		setcolor(TIPS);
		settextstyle(BOLD_FONT, HORIZ_DIR, 2);
		outtextxy(w/2 + 30, 850, mess);
//		outtextxy(1200 + textwidth(ThongBao) + 40, 750, mess);
	}else{
		setcolor(TIPS);
		outtextxy(1200, 350, "Khong tim thay sach nay!");
	}
}
...
void DrawMuonSach(){
	...
	setcolor(TEXT_TITLE_COLOR);
	outtextxy(w/2 - textwidth(MuonSach)/2, 125, MuonSach);
...
void DrawTraSach(){
	...
	settextstyle(BOLD_FONT, HORIZ_DIR, 5);
	setcolor(TEXT_TITLE_COLOR);
	outtextxy(w/2 - textwidth(TraSach)/2, 125, TraSach);
...
void DrawTopTen(){
	...
	settextstyle(BOLD_FONT, HORIZ_DIR, 5);
	setcolor(TEXT_TITLE_COLOR);
	outtextxy(w/2 - textwidth(TopTen)/2, 130, TopTen);
	...
	setlinestyle(SOLID_LINE,0, 3);
	setcolor(BORDER_COLOR);
	rectangle(XTT[0], 300, XTT[8], 980);
	line(XTT[0], 380, XTT[8], 380);
	for(int i=0; i<8; i++){
		line(XTT[i], 300, XTT[i], 980);
		outtextxy((XTT[i+1]+XTT[i])/2 - textwidth(t[i])/2, 340-textheight(t[i])/2, t[i]);
	}
...
//	topsach.sort();
	...
	for(int i=0; i < (topsach.n < 10 ? topsach.n : 10); i++){
		itoa(i+1, ch, 10);
		outtextxy((XTT[0]+XTT[1])/2-textwidth(ch)/2, 400 + i*50, ch);
		outtextxy(XTT[1]+10, 400 + i*50, DSDS.nodes[topsach.list[i].indexDS]->ISBN);
		outtextxy(XTT[2]+10, 400 + i*50, DSDS.nodes[topsach.list[i].indexDS]->tensach);
		itoa(DSDS.nodes[topsach.list[i].indexDS]->sotrang, ch, 10);
		outtextxy(XTT[3]+20, 400 + i*50, ch);
		outtextxy(XTT[4]+10, 400 + i*50, DSDS.nodes[topsach.list[i].indexDS]->tacgia);
		itoa(DSDS.nodes[topsach.list[i].indexDS]->nxb, ch, 10);
		outtextxy((XTT[5]+XTT[6])/2-textwidth(ch)/2, 400 + i*50, ch);
		outtextxy(XTT[6]+10, 400 + i*50, DSDS.nodes[topsach.list[i].indexDS]->theloai);
		itoa(topsach.list[i].cnt, ch, 10);
		outtextxy((XTT[7]+XTT[8])/2-textwidth(ch)/2, 400 + i*50, ch);
	} 
}
